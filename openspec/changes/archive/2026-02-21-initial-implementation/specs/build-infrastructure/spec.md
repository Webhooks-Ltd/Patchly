...
All packages in the solution SHALL share the same version number, determined at CI time by the `paulhatch/semantic-version` GitHub Action.

#### Scenario: Version is injected at build time
- **WHEN** the CI pipeline runs
- **THEN** the `paulhatch/semantic-version` action computes the version from commit history
