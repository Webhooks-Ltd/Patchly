...
### Requirement: ApplyTo Method

**Reason**: Deferred to v2. The target type resolution design (convention vs attribute vs generic) needs further consideration. Shipping `WasProvided` and `Provided` accessor gives developers full manual control while the `ApplyTo` design matures.

**Migration**: Use `WasProvided(string)` or the `Provided` accessor to manually apply changes:
```csharp
