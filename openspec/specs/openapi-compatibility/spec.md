...
For .NET 9+ projects using the built-in OpenAPI document generation, Patchly schemas SHALL be correct without additional packages.

#### Scenario: Built-in OpenAPI respects JsonIgnore

- GIVEN a .NET 9+ project using `builder.Services.AddOpenApi()`
- AND a `[PatchDocument]` class with `[JsonIgnore]` on tracking members
- WHEN the OpenAPI document is generated at `/openapi/v1.json`
- THEN tracking members do not appear in the schema

#### Scenario: Built-in OpenAPI shows nullable properties

- GIVEN a .NET 9+ project using built-in OpenAPI
